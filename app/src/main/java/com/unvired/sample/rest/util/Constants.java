package com.unvired.sample.rest.util;

/**
 * Created by nishchith on 24/07/17.
 */

public class Constants {
    // App Name
    public static final String APP_NAME = "UnviredRESTSample";

    // BE Names
    public static final String BE_WEATHER = "WEATHER";


    // PA Function Names
    public static final String PA_GET_WEATHER = "UNVIREDRESTSAMPLE_PA_GET_WEATHER"; // Get Weather

    public static final String UNVIRED_APP_PREFERENCE_KEY = "UNVIRED_APP_PREFERENCE_KEY";
    public static final String APPLICATION_NAME = "UnviredDBSample";

    public static final int MODE_HOME = 0;
    public static final int MODE_GET = 1;

    public static final String RESPONSE_CODE_ERROR = "ERROR";
    public static final String RESPONSE_CODE_SUCCESSFUL = "SUCCESS";


}
